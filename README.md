...
[![CI](https://github.com/lian-automation/Virtual-Smart-Factory/actions/workflows/ci.yml/badge.svg?branch=main)](https://github.com/lian-automation/Virtual-Smart-Factory/actions/workflows/ci.yml)
...
> 🔎 **数字溯源**：README / 简历中每个指标的生成命令与输出位置见 [REPRODUCE.md](REPRODUCE.md)，欢迎 clone 复跑。
...
| 事件总线 | `core/event_bus.py` | 发布/订阅（精确+通配）；JSONL 追加持久化（按大小轮转+保留最近 N 份，磁盘满自动降级不停仿真）；环形缓冲查询 `recent()/replay()` |
...
#   --host 地址        三协议监听地址（默认 127.0.0.1 仅本机；0.0.0.0 局域网开放）
#   --allow-write      Modbus DO/AO 写回开关（默认只读）
